...
pub fn binomial_price(instrument: &VanillaOption, s0: f64, r: f64, sigma: f64, n: usize) -> f64 {
    ...
    let dt = time_to_maturity / n as f64;  // Time step
    let up = (sigma * dt.sqrt()).exp();  // Upward movement factor
    let down = 1.0 / up;                 // Downward movement factor
    let p = (r.exp() * dt - down) / (up - down); // Risk-neutral probability
